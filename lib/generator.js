 ...
 * @return {Object}
 */
var convert = function(descriptor, mongoose) {
    ...
    var decoded = JSON.parse(encoded, function(key, value) {
        ...
        if (typeof value == "object" && value.schema) {
            if (!mongoose) throw new Error("mongoose cannot be null");

            let schema = mongoose.model(value.schema).schema;
            if (value.extend && typeof value.extend == "object") {
                schema.add(convert(value.extend, mongoose));
            }
            return schema;
        }
        return value;
