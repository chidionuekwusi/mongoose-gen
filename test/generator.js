...
describe("mongoose-gen integration", function() {
  ...
  describe("register custom methods", function() {
        ...
        var fn = function() {
          switch (type) {
            ...
            case "Boolean":
              return true;
            case "Date":
              return new Date(111111111111111);
            case "Buffer":
              return new Buffer("default");
            case "ObjectId":
              return 1;
            case "Mixed":
              return {};
    ...
    it("._get() should throw an error when the required modifier is not found", function() {
      assert.throws(
        ...
        Error,
        "Throws error when validator was not registered previously"
      );
    });
    ...
    it("._matchType() should throw when an unknown type is requested", function() {
      assert.throws(
        ...
        Error,
        "NumberLong is not yet defined as a type"
      );
    });
    ...
    it("._check() should throw when regexp param is not string", function() {
      assert.throws(
        ...
        Error,
        "Param for match is not string to be used with RegExp"
      );
    });
  ...
  describe(".convert()", function() {
    it("should generate a new Schema instance", function() {
      ...
      assert.ok(
        TestSchema instanceof mongoose.Schema,
    ...
    it("should store the model in the database", function(done) {
      ...
      instance.save(function(err) {
        ...
        assert.ok(
          _.isString(instance.id),
          "instance now has a " + "String id, which means it was saved on disk"
        );
  ...
  describe("DBRef support", function() {
    ...
    it(
      "should store data for both normally created models and " +
        "`mongoose-gen` created models the same way",
      function(done) {
        var _this = this;
        ...
        Q()
          ...
          .spread(function(plain, gen) {
            ...
            assert.equal(
              gen.plain,
              plain.id,
              "should have linked the other document"
            );
  ...
  describe("nested", function() {
    it("should enable deeply nested documents", function(done) {
      ...
      var BlogPostModel = (this.BlogPostModel = mongoose.model(
        "BlogPost",
        BlogPostSchema
      ));
      ...
      var data = {
        ...
        comments: [{ text: "This is marvelous!" }, { text: "Amazing!" }]
      };
  ...
  describe("getSchema()", function() {
    it("should work just like the new api convert()", function(done) {
      ...
      assert.ok(
        ProductSchema instanceof mongoose.Schema,
        "should be instance of mongoose.Schema"
      );

      var ProductModel = (this.ProductModel = mongoose.model(
        "product",
        ProductSchema
      ));
      var productData = {
